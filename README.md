...
### v4.9.1
- **Vesktop / Electron client mods no longer skip `GAME`/`STREAM` quests** &mdash; `IS_DESKTOP` only checked for `window.DiscordNative`, which Vesktop doesn't expose (it loads Discord's web build inside Electron and exposes `VesktopNative` instead &mdash; the same reason Discord shows Vesktop users as "Web"). So every play/stream quest hit the `"requires desktop app. Skipping."` guard even though the fake-`RunningGameStore` completion path works fine there. Detection now also accepts `window.VesktopNative` and an `Electron/` user-agent, covering Vesktop, Equicord, and Legcord. Fixes [#35](https://github.com/nyxxbit/discord-quest-completer/issues/35).
- **Vencord Webpack errors no longer abort the whole loader** &mdash; when Vencord is present, Orion extracts modules through Vencord's Webpack API first. On some builds that API throws internally (`Cannot read properties of undefined (reading 'c')` &mdash; its `wreq` module cache isn't ready), and because the Vencord branch had no inner `try/catch` the exception propagated to the top-level handler &rarr; `"Failed to load Discord modules. Aborting."`, even though the guarded native-webpack fallback would have succeeded. The Vencord branch is now isolated: any throw logs a warning and falls through to native extraction. Fixes [#34](https://github.com/nyxxbit/discord-quest-completer/issues/34).
